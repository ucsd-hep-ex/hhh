#!/usr/bin/env python3
"""
Mass-agnostic chi2 baseline for resolved jet assignment.

Assigns jets to resonance daughters by minimizing chi2 with respect to the
mean dijet mass (no fixed mass hypothesis). Compatible with any topology
defined in the event file (e.g. HHH->6b, ss->4g).

Usage:
  python -m src.models.mass_agnostic_baseline \\
    --input-file /path/to/test.h5 \\
    --event-file /path/to/event_files/assign_resolved_hhh_regress_mass.yaml \\
...
def chi2_mean_mass(jets, seg_arr):
    """Chi2 with respect to mean dijet mass (mass-agnostic)."""
    dijet_masses = (np.sum(jets[:, seg_arr], axis=3)).m  # (n_events, n_seg, n_pairs)
    m_mean = np.mean(dijet_masses, axis=2)  # (n_events, n_seg)
    error = np.where(m_mean > 0, m_mean, 1.0)  # avoid div by zero
    return np.sum(
        (dijet_masses - m_mean[:, :, np.newaxis]) ** 2 / error[:, :, np.newaxis] ** 2,
        axis=2,
    )


def run_baseline(jets, n_pairs: int = 3):
    """Run mass-agnostic chi2 assignment. Returns best indices (n_events, n_pairs, 2)."""
    n_jets = jets.shape[1]
    seg_arr = build_seg_arr(n_jets, n_pairs)
    measure = chi2_mean_mass(jets, seg_arr)
...
def make_plots(results: dict, output_dir: Path, event_config: dict):
    ...
    ax.text(
        0.98, 0.95,
        f"Mass-agnostic $\\chi^2$ baseline\n{event_config['topology_label']}",
    ...
    fig.savefig(output_dir / "mass_agnostic_baseline_efficiency.pdf")
    plt.close(fig)
    ...
    fig.savefig(output_dir / "mass_agnostic_baseline_purity.pdf")
    plt.close(fig)


def main():
    parser = argparse.ArgumentParser(
        description="Mass-agnostic chi2 baseline for resolved jet assignment.",
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Event YAML file defining topology (e.g. assign_resolved_hhh_regress_mass.yaml).",
    )
    parser.add_argument(
        "--output-dir",
        "-o",
        type=Path,
        default=None,
        help="Output directory for efficiency/purity plots and prediction H5.",
    )
    parser.add_argument(
        "--tag",
        type=str,
        default="mass_agnostic_baseline",
        help="Subdir under output_dir: output_dir/{tag}/ with plots and chi2_prediction.h5",
    )
    parser.add_argument(
        "--max-events",
        type=int,
    ...
    print("Running mass-agnostic chi2 baseline...")
    indices_min = run_baseline(jets, n_pairs=n_pairs)
    ...
    if args.output_dir:
        out_subdir = args.output_dir / args.tag
        out_subdir.mkdir(parents=True, exist_ok=True)
        make_plots(results, out_subdir, event_config)
        pred_path = out_subdir / "chi2_prediction.h5"
        save_prediction_h5(
            pred_path, args.input_file, indices_min, event_config
        )
        print(f"Outputs saved to {out_subdir}")
