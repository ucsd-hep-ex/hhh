...
def get_datasets(arrays, n_higgs):  # noqa: C901
    ...
    h1_bs = ak.ones_like(mask) * -1
    h2_bs = ak.ones_like(mask) * -1
    if n_higgs == 3:
        h3_bs = ak.ones_like(mask) * -1

    # index of large-radius jet if Higgs is reconstructed
    h1_bb = ak.ones_like(fj_mask) * -1
    h2_bb = ak.ones_like(fj_mask) * -1
    if n_higgs == 3:
        h3_bb = ak.ones_like(fj_mask) * -1
